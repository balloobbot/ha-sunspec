...
**modbus-connection 4.8.1** (tmodbus backend) and keeps pysunspec2 only for the
...
author-time, and its SunSpec answer to "many models" is an _offline_ code
...
| `0`      | _divide the leftover model length by the block size_ | `repeating_group(<int>, …)`        |
...
**The count point is always a point of the _model_, never of the group that
...
calls a point real only if the device implemented it _and_ it carries a unit (or
...
deliberately do _not_ pass an `IntEnum` to `enum16()`.
...
curve _points_ inside curves; those exist in the map, are sized correctly, and
...
What it _does_ do is use public API in ways the docs don't cover:
...
- **The readable map is discovered, not declared.** `register_ranges` is normally
  written into a component class by its author. Here the map is a _result of the
  scan_: a SunSpec chain is one contiguous run of registers — each model's header
...
`tests/test_data/inverter.json`, that is a _synthetic_ pysunspec2 fixture and is
called out as such.
...
are solid. The repo's _hand-written_ analysis is weaker: it is a heavily
...
echo carrying the _stored_ value rather than an exception. That second point is
...
multi-register write. Which also means a write planner must _not_ merge as freely
...
- the _scale factor_ is unimplemented → `None` (wrong)
...
plainly.** `tests/test_data/inverter.json` is a _synthetic_ pysunspec2 fixture
...
_or_ out of the -10..10 range — i.e. it agrees with modbus-connection, not with
pysunspec2.
...
### 2. `scale_in_block` should be per field, not per component — _no real-world case_

It is a class attribute, so a repeating block carries _all_ its scale factors or
...
Recording this as **not worth doing**. 63001 and 63002 are SunSpec's own _test_
...
real, and it references _only_ its own — which `scale_in_block=True` already
...
another repeat, instance _i_ reads its count from `count_addr + i * stride`.
...
scale registers default to _not_ moving, which is the safer default. Counts have
...
The same root cause makes a group that _follows_ a runtime-counted group
...
### 5. Let a `ManualComponent` join a `ComponentGroup` — _done in 4.4.0_
...
### 6. The runtime-built component stays unsupported — _by decision_
